...
typedef enum { eStandard,
               eFixed,
               eDrone } eEdge;
typedef enum { mNormal,
               mSlew,
               mEdge,
               mSelect,
               mBank } eMode;
...
void gpio_set_gpio_pin(u8 pin) {
  ...
  if (0) {
    ...
    char cmd[6] = { JF_VOX, 0, 0, 0, 0, 0 };
    u16 a = aout[3].target<<2;
    u16 b = aout[0].target<<2;
...
void spi_selectChip(u8 dac, u8 spi) {
  ;
  ;
}
void spi_unselectChip(u8 dac, u8 spi) {
  ;
  ;
}
void spi_write(u8 dac, u8 spi) {
  ;
  ;
}
...
static void aout_write(void) {
  ...
  for (u8 i1 = 0; i1 < 4; i1++) {
    ...
    Wire.write(cmd, 4);
    Wire.endTransmission();
    // Serial.printf("==> Wired[%d] = %d, %d\n", i1, aout[i1].now, (cmd[2]<<8) + cmd[3]);
  }

  if (0) {
    Serial.printf("==> Wire[%d] = %d\n", 3, aout[3].now);
    char cmd[6] = { JF_NOTE, 0, 0, 0, 0, 0 };
    u16 a = aout[3].now << 2;
    u16 b = SEMI[60];
    Wire.beginTransmission(JF_ADDR);
    // cmd[1] = 1;
    ...
    Wire.write(cmd, 5);
    Wire.endTransmission();
    // Serial.printf("==> Wire[%d] = %d, %d\n", i1, aout[i1].now, (cmd[2]<<8) + cmd[3]);
  }

  if (0) {
    char cmd[6] = { JF_PITCH, 0, 0, 0, 0, 0 };
    u16 a = 1;
    u16 b = aout[3].now << 2;
    Wire.beginTransmission(JF_ADDR);
    cmd[1] = a >> 8;
...
void flash_read(void) {
  ...
  File preset_file = SD.open("ES.BIN", FILE_READ);
...
void setup(void) {
  ...
  if (flash_is_fresh()) {
    ...
    for (i1 = 0; i1 < 8; i1++) {
      memcpy((void *)&flashy.es[i1], &es, sizeof(es));
    ...
    switch (buffer[0]) {
      ...
      case 0x78:  // x
        if (a > 0) {
          ...
          // check_flashy();
          // for(i2=0;i2<8;i2++)
          //   glyph[i2] = flashy.glyph[preset_select][i2];
        }
        break;
    }
  }
